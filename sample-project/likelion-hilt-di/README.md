# likelion-hilt-di — Hilt 샘플

**경로:** `archive/sample-project/likelion-hilt-di`

**Jetpack Compose + MVVM** 기반 **Firebase(Firestore) 채팅 앱**입니다.  
`likelion-non-di`, `likelion-manual-di`와 **기능은 같고**, **Hilt**로 ViewModel·Repository를 주입합니다.
...
│   │   ├── model/
│   │   └── repository/
│   ├── di/              # Hilt 모듈 (예: FirebaseAuthHiltModule)
│   ├── navigation/
│   ├── ui/
│   └── util/
└── features/
...
- **ViewModel** — `LauncherViewModel`, `SignViewModel`, `ChatRoomListViewModel`, `ChatRoomViewModel`, `SettingViewModel` 등 `@HiltViewModel`
- **모듈**
...
- Hilt로 ViewModel/Repository 주입 구조 익히기
