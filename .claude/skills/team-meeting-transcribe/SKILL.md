...
11. **🗂️ 보드 PDF + mom + commit 정합성** (v2.2 갱신):
    - **사용자 수기 mom** (`teams-docs/<X>team/mom/<YYMMDD>*`)에서 멤버별 분담 추출
    - **GitHub Project 보드 PDF** (`teams-docs/<X>team/backlog/backlog_<YYMMDD>_<am|pm>.pdf`) Read 도구로 파싱
      - 컬럼별 카드 수 + WIP 한도 위반 검사 (예: "In progress 7/3" → 4건 초과)
      - 각 카드: #번호, [Issue X-N] 라벨, 제목, assignee
    - 회의에서 합의된 작업이 mom + 보드 + 24h commit과 일치하는지 3중 매칭
    - §🗂️ 매칭표 갱신 + 정합성 신호 발현 시 §🚨 강사 사전 통지 후보
12. **인용할 핵심 발화** — 1~2개 (스냅샷에 인용 블록으로 직접 삽입)
...
| 🗂️ 보드 In Progress + 분담 cross-check ⭐ v2.2 | **사용자 수기 mom + 보드 PDF (`backlog/backlog_<YYMMDD>_<am\|pm>.pdf`) + 24h commit 3중 매칭표** + WIP 한도 위반 + 정합성 신호 (R-stall / R-quiet / R-WIP / R8) 식별 |
...
- **🗂️ 보드 cross-check에서 정합성 신호 ≥ 3건 발현** ⭐ v2.1 신규 (R-stall / R-quiet / 미배정 멤버 / R-WIP 등이 3개 이상이면 회의록 별도 작성)
